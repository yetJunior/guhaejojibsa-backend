...
@ActiveProfiles("test")
@Transactional
public class CardReceiptModuleServiceTest {

    @Autowired
    private CardReceiptModuleService cardReceiptModuleService;
    @Autowired
    private CardReceiptRepository cardReceiptRepository;
    @Autowired
    private ReceiptRepository receiptRepository;

    @DisplayName("카드 영수증 정보 조회 - 성공 케이스")
    @Test
    void findByReceipt_SuccessTest() {
        // Given
        ...
        // When
        CardReceipt result = cardReceiptModuleService.findByReceipt(savedReceipt);

        // Then
        assertNotNull(result);
        assertEquals(savedReceipt, result.getReceipt());
    }
